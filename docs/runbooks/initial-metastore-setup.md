# Runbook: Initial Metastore Setup (One-Time)

These steps are required **once**, after the Databricks UC Metastore is first deployed.
The grants and groups are attached to the Metastore itself — they persist as long as the
Metastore exists.

> In this platform the UC Metastore is an account-level resource that survives `workload-dbx`
> destroy/recreate cycles. You do **not** need to repeat these steps after each destroy.
> Only repeat them if the Metastore itself was fully destroyed and recreated from scratch.
...
## Step 2 — Create Account-Level Groups (before or after workload-catalog)
...
> **Note:** The `workload-catalog` job attempts all GRANTs and emits a `WARNING` for any group
> that does not exist yet — it does **not** fail. Re-run `workload-catalog` after creating the
> groups to apply the deferred grants (all GRANT statements are idempotent).
...
3. **Step 1 — SP grants** (run before workload-catalog)
4. **Step 2 — Account-level groups** (create before or after workload-catalog; re-run catalog job after creation)
